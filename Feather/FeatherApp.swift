...
struct AppMasterApp: App {
	...
	var body: some Scene {
		WindowGroup {
			...
			// dear god help me
			.onAppear {
				if let style = UIUserInterfaceStyle(rawValue: UserDefaults.standard.integer(forKey: "AppMaster.userInterfaceStyle")) {
					UIApplication.topViewController()?.view.window?.overrideUserInterfaceStyle = style
				}
				
				UIApplication.topViewController()?.view.window?.tintColor = UIColor(Color(hex: UserDefaults.standard.string(forKey: "AppMaster.userTintColor") ?? "#848ef9"))
			}
		}
	}
	
	private func _handleURL(_ url: URL) {
		if url.scheme == "appmaster" {
			/// appmaster://import-certificate?p12=<base64>&mobileprovision=<base64>&password=<base64>
			...
			/// appmaster://export-certificate?callback_template=<template>
			...
			/// appmaster://source/<url>
			if let fullPath = url.validatedScheme(after: "/source/") {
				FR.handleSource(fullPath) { }
			}
			/// appmaster://install/<url.ipa>
			if
...
class AppDelegate: NSObject, UIApplicationDelegate {
	...
	private func _createPipeline() {
		...
		let pipeline = ImagePipeline {
			...
			let dataCache = try? DataCache(name: "com.appmastr.app.datacache") // disk cache
	...
	private func _addDefaultCertificates() {
		guard
			UserDefaults.standard.bool(forKey: "appmaster.didImportDefaultCertificates") == false,
		...
		do {
			...
			UserDefaults.standard.set(true, forKey: "appmaster.didImportDefaultCertificates")
		} catch {
